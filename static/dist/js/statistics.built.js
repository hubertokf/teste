...
function processSeen(seen) {
    ...
    for (i = seen.pokemon.length - 1; i >= 0; i--) {
        ...
        lastSeen = lastSeen.getHours() + ':' + ('0' + lastSeen.getMinutes()).slice(-2) + ':' + ('0' + lastSeen.getSeconds()).slice(-2) + ' ' + lastSeen.getDate() + ' ' + monthArray[lastSeen.getMonth()] + ' ' + lastSeen.getFullYear();
...
function appearanceTab(item) {
    ...
    return loadAppearancesTimes(item['pokemon_id'], item['spawnpoint_id']).then(function (result) {
        $.each(result.appearancesTimes, function (key, value) {
            ...
            saw = saw.getHours() + ':' + ('0' + saw.getMinutes()).slice(-2) + ':' + ('0' + saw.getSeconds()).slice(-2) + ' ' + saw.getDate() + ' ' + monthArray[saw.getMonth()] + ' ' + saw.getFullYear();
...
//# sourceMappingURL=statistics.built.js.map
